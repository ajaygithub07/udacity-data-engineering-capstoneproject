...
def process_immigration_data(spark, input_data, output_data, file_name, temperature_file, mapping_file):
    ...
    immigration_df = utils.clean_spark_immigration_data(immigration_df)

    # create visa_type dimension table
    visatype_df = etl_capstone_library.create_visa_type_dimension_table(immigration_df, output_data)

    # create calendar dimension table
    calendar_df = etl_capstone_library.create_immigration_calendar_dimension(immigration_df, output_data)

    # get global temperatures data
    temp_df = process_global_land_temperatures(spark, input_data, temperature_file)

    # create country dimension table
    dim_df = etl_capstone_library.create_country_dimension_table(spark, immigration_df, temp_df, output_data, mapping_file)

    # create immigration fact table
    fact_df = etl_capstone_library.create_immigration_fact_table(spark, immigration_df, output_data)
...
def process_demographics_data(spark, input_data, output_data, file_name):
    ...
    new_demographics_df = utils.clean_spark_demographics_data(demographics_df)

    # create demographic dimension table
    df = etl_capstone_library.create_demographics_dimension_table(new_demographics_df, output_data)
...
def process_global_land_temperatures(spark, input_data, file_name):
    ...
    new_temperature_df = utils.clean_spark_temperature_data(temperature_df)
...
def main():
    ...
    # Uncomment following code to run on EMR 
    input_data = "s3://udacity-de-sparkify-data-lake/data/"
    output_data = "s3://udacity-de-sparkify-data-lake/output/"
    
    # Uncomment the following code to run on local project workspace 
    # Make sure all files are present in this directory
    # My most of the testing is done from S3 location 
